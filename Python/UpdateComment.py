...
r = requests.get('https://jira.fr/rest/api/2/search?jql=project in ("Test") and comment ~ "Word%26Gil"&maxResults=500',auth=(Username,Password), verify=False)
print(r.status_code)
data = r.json()
...
for i in data["issues"]:
    ...
    for j in data_c["fields"]["comment"]["comments"]:
        ...
        if result and data_c["key"] not in issueLIst:
            count += 1
            newCommentBody = re.sub(r'Word&Gyr', "Word+Gyr", j["body"]) #fonction permettant de modifier un élément dans une texte et renvoi ce texte avec l'élément modifié
            payload = { 
