...
def jiraAdminUser = ComponentAccessor.getUserManager().getUserByName("jira-admin")
...
String sendEmail (String message){
    ...
    Email email = new Email("test@fr")
	email.setMimeType("text/html")
    email.setCc("")
    email.setSubject("Job transition automatique en erreur")
    email.setBody("""
    				Bonjour,<br>	                    
                    <br>Le job permettant le passage de transition du projet s'est terminé avec une erreur le ${today}.<br>
...
def query_2 = jqlQueryParser.parseQuery("myquery")

def search = searchService.search(jiraAdminUser, query, PagerFilter.getUnlimitedFilter())
def search_2 = searchService.search(jiraAdminUser, query_2, PagerFilter.getUnlimitedFilter())
...
try{
    ...
    log.warn("Clé de ticket : " + i.getKey())
    ...
    log.warn("Statut des US rattaché au ticket ${i.getKey()} " + statusName)
    ...
    if(value.size() == statusName.size()){
        ...
        def validationTransition = issueService.validateTransition(jiraAdminUser, currentKey.getId() , transition ,  issueService.newIssueInputParameters())
                        if(validationTransition.isValid()){
                            //ici on execute la transtition

                            issueService.transition(jiraAdminUser, validationTransition)
...
try{
    ...
    log.warn("Statut des US rattaché au ticket ${i.getKey()} " + statusNameFor_Us_Use)
