...
def projectGJA = ComponentAccessor.getProjectManager().getProjectByCurrentKey("ProjectKey")
def projectId = projectGJA.getId()


def allComponentJA = projectComponent.findAllForProject(projectId)
def componentWithoutProject = []
def componentWithoutlead = []
def componentWithoutDescription = []

//Composant sans description
for(i in allComponentJA){
